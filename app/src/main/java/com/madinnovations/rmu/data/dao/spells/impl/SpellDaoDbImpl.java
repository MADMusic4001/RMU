...
public class SpellDaoDbImpl extends BaseDaoDbImpl<Spell> implements SpellDao, SpellSchema {
	...
	protected ContentValues getContentValues(Spell instance) {
		...
		values.put(COLUMN_SPELL_LEVEL, instance.getSpellLevel());
		...
		values.put(COLUMN_RR_MOD, instance.getResistanceRollMod());
	...
	private Spell cursorToEntity(@NonNull Cursor cursor, SpellList spellList) {
		...
		instance.setSpellLevel(cursor.getShort(cursor.getColumnIndexOrThrow(COLUMN_SPELL_LEVEL)));
		...
		instance.setResistanceRollMod(cursor.getShort(cursor.getColumnIndexOrThrow(COLUMN_RR_MOD)));
